...
const LLAMA_NODES_URL = 'https://eth.llamarpc.com';
const TENDERLY_URL = `https://mainnet.gateway.tenderly.co/${TENDERLY_KEY}`
const QUICKNODE_URL = `${QUICKNODE_ENDPOINT}`;
const GASHACK = 'https://beta-be.gashawk.io:3001/proxy/rpc';
const b69 = 'https://rpc.builder0x69.io/';

const RUN_TIME = 1 * 60 * 1000; // 5 minutes
const CONCURRENT_REQUESTS = 100; // 1,000
...
const testSpeed = async (providerUrl) => {
  if (providerUrl.length == 0) return 'No provider';
...
(async function main() {
  //   // Speed tests
    ...
    await testSpeed(CHAINSTACK_URL).then((result) => {
      console.log("sharedtools Speed: ", result);
    });
    await testSpeed(LLAMA_NODES_URL).then((result) => {
      console.log("Llama Nodes Speed: ", result);
    });

    await testSpeed(GASHACK).then((result) => {
      console.log("GasHawk Nodes Speed: ", result);
    });

    await testSpeed(b69).then((result) => {
      console.log("builder 69 Nodes Speed: ", result);
    });
  ...
  //   console.log("SGT Concurrent Requests: ", result);
  // });
